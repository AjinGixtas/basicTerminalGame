...
[color={Util.CC(Cc.w)}]Batch distribution:[/color] ({GenerateBatchDistVisual(hackfarm)} )";
	...
	string[] GenerateAppendableBox(BotFarm hackfarm, int index) {
		...
		box[1] = $"     {Util.Format($"{Util.MINERAL_PROFILES[hackfarm.mineralDistribution[index].Item1].Shorthand}", StrType.G_MINERAL, $"{hackfarm.mineralDistribution[index].Item1}")}     {I}";
	...
	string GenerateBatchDistVisual(BotFarm hackfarm) {
		string result = "";
		for (int i = 0; i < hackfarm.mineralDistribution.Length; ++i) {
			result += $" +{Util.Format($"{hackfarm.mineralDistribution[i].Item2 * hackfarm.BatchSize}", StrType.NUMBER, "1")}[color={Util.CC(Cc.w)}]u[/color]{Util.Format($"{Util.MINERAL_PROFILES[hackfarm.mineralDistribution[i].Item1].Shorthand}", StrType.G_MINERAL, $"{hackfarm.mineralDistribution[i].Item1}")}";
		}
		return result;
	}
