...
public partial class ItemSaleBox : Control {
    ...
    public void Render() {
		double percentDiffer = ((ItemSeller.PriceModels[itemID].value - ItemCrafter.ALL_ITEMS[itemID].Value) / ItemCrafter.ALL_ITEMS[itemID].Value * 100);
		int deltaCode = ItemSeller.PriceModels[itemID].value > ItemSeller.PriceModels[itemID].pastValue ? 1 : ItemSeller.PriceModels[itemID].value < ItemSeller.PriceModels[itemID].pastValue ? -1 : 0;
		string arrow = deltaCode == 1 ? "▴" : deltaCode == -1 ? "▾" : "-";
		CurrentPriceDisplay.Text = Util.Format($"{ItemSeller.PriceModels[itemID].value}", StrSty.NUMBER).PadLeft(35)
    ...
    public void OnSellItems() {
		...
		PlayerDataManager.DepositGC((long)(ItemSeller.PriceModels[itemID].value * amount));
	}
}
