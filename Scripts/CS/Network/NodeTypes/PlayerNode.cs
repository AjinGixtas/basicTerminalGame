...
public class PlayerNode : ScriptedNetworkNode {
    ...
    async void TutorialSpeak0() {
        ...
        cm.Enqueue($"I need you to {Util.Format("link <sector_name>", StrSty.CMD_FUL)}");
        cm.Enqueue($"See all sectors with {Util.Format("sector", StrSty.CMD_FUL)}");
        cm.Enqueue("Alright, you should be good now. Cya.");
    ...
    async void TutorialSpeak1(string cmd, Dictionary<string,string> farg, string[] parg) {
        if (!(cmd == "link" && parg.Length > 0 && parg[0].Contains(TutorialSector.TUTORIAL_SECTOR_NAME))) {
    ...
    async void TutorialSpeak2(string cmd, Dictionary<string, string> farg, string[] parg) {
        ...
        if (!NetworkManager.GetSectorNames(true).Contains(TutorialSector.TUTORIAL_SECTOR_NAME)) {
            cm.Enqueue(7, "...");
            ...
            cm.Enqueue(7, "...");
            cm.Enqueue($"Run `{Util.Format($"link {TutorialSector.TUTORIAL_SECTOR_NAME}; scan;", StrSty.CMD_FUL)}`");
        } else {
            await Task.Delay(500);
