...
public partial class TerminalSidebar : MarginContainer {
	...
	public int pageAmount => animalSymbol.Length;
	int pageIndex = 0; public int PageIndex {
		get => pageIndex;
		set {
            if (value < 0 || value >= pageAmount) {
                GD.PrintErr($"Invalid page index: {value}. Must be between 0 and {pageAmount - 1}.");
                return;
            }
			statSwitchTimer.Start();
            pageIndex = value % pageAmount;
        }
	}
	bool doSwitch = true; public bool DoSwitch {
        get => doSwitch;
        set {
            doSwitch = value;
            if (doSwitch) statSwitchTimer.Start();
            else statSwitchTimer.Stop();
        }
    }
	public void OnStatSwitchTimerTimeout() {
		if (!DoSwitch) return;
        PageIndex = (PageIndex + 1) % 4;
		artDiplayer.Text = $"[color=cyan]{animalSymbol[PageIndex % animalSymbol.Length]}[/color]";
	}
	public override void _Process(double delta) {
		if (!Visible) return;
		playerStatDisplayer.Text = $"[color=cyan]{statFormater[PageIndex % animalSymbol.Length]()}[/color]";
	}
...
{sb.ToString().TrimEnd()}";
	...
	string GetRavenStatFormat() {
		...
		foreach (BotFarm bot in NetworkManager.BotNet) {
			sb.Append($"[color={Util.CC(ItemCrafter.ALL_ITEMS[bot.mineralDistribution[0].Item1].ColorCode)}{(Mathf.RoundToInt(Mathf.Pow(bot.CycleTimeRemain / bot.XferDelay * 1.189207115002721, 32))).ToString("X2")}]█[/color]");
		}
		return
